...
| `phase0-lock-test.sh` | `scripts/phase0-lock-test.sh` | Phase 0 verification (ADR-003) for the on-disk lock primitive: 5 concurrent node workers each acquire+hold+release on a shared dir; asserts strict serialization (no overlapping intervals) and minimum total span. Backed by `.github/workflows/phase0-lock-test.yml`. | `node`, `bash`, compiled `server/dist/cache.js` | Y |
...
| `check-vendor-drift.sh` | `scripts/check-vendor-drift.sh` | Re-fetches vendored modules from upstream at the pinned SHA in ATTRIBUTION.md and diffs (modulo header strip + import-extension normalization). Exit 1 on any drift; consumed by `.github/workflows/vendor-drift.yml` (Mondays 12:00 UTC) | `curl`, `diff`, `awk`, `sed` | Y |
...
| `inject-reminders.sh` | `scripts/inject-reminders.sh` | Hotskills hook script invoked from `PostCompact`, `SessionStart`, `UserPromptSubmit` per ADR-005. Reads merged config + state, emits `<system-reminder>` blocks (activated-skills always when non-empty, opportunistic per ADR-005 lifecycle), atomically updates `state.json`. All IO failures log to `${HOTSKILLS_CONFIG_DIR}/logs/hook.log`; never blocks prompts (always exits 0). | `bash`, `jq`, POSIX coreutils | Y |
| `inject-reminders.test.sh` | `scripts/tests/inject-reminders.test.sh` | Bash-driven test harness for `inject-reminders.sh`: 14 scenarios covering missing dir, empty allow-list, single + 25-skill rendering, opportunistic lifecycle on all 3 events, corrupted config, runtime budget, description truncation, and merged dedup. | `bash`, `jq` | Y |
