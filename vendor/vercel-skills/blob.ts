// vendored-from-START
// Source: https://github.com/vercel-labs/skills (src/blob.ts)
// Sync SHA: bc21a37a12b90fcb5aec051c91baf5b227b704b1 (tag v1.5.1)
// Sync date: 2026-04-23
// License: MIT (see ./LICENSE)
// Modifications:
//   1. parseFrontmatter inlined from upstream src/frontmatter.ts (which is
//      NOT vendored — see ./patches/blob-inline-frontmatter.patch).
//      Adds runtime dep on the 'yaml' package (server/package.json).
//   2. Import path `'./types.ts'` rewritten to `'./types.js'` for
//      ESM/NodeNext resolution under hotskills' tsconfig.
// vendored-from-END
...
// ─── Inlined frontmatter parser (upstream src/frontmatter.ts) ───

/**
 * Minimal frontmatter parser. Only supports YAML (the `---` delimiter).
 * Intentionally does NOT support `---js` / `---javascript` blocks (which
 * gray-matter's built-in JS engine would otherwise execute as code) — this
 * is a deliberate hardening choice inherited from upstream.
 *
 * Inlined from upstream `src/frontmatter.ts` to keep the vendored module
 * list to the five files explicitly enumerated in ADR-002.
 */
